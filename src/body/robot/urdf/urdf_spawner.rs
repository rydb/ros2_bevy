...
pub fn handle_new_urdf_roots(mut commands: Commands, mesh_server: Res<Assets<Mesh>>, new_urdfs: Query<(Entity, &UrdfRoot)>) {
    ...
    for (e, urdf) in new_urdfs.iter() {
        ...
        for link in &urdf.links {
            ...
            for visual in &link.visual {
                ...
                let id = cmd.id();
                model.add_bevy_components(cmd, mesh_server);
