...
impl Plugin for TransformWidgetPlugin {
    fn build(&self, app: &mut App) {
        ...
        .add_systems(Update, (widget_spawn_for_selected, transform_widget_behaviour,
            manage_x_tugs, manage_y_tugs, manage_z_tugs,
            manage_y_rings, manage_z_rings).before(widget_despawn_for_deselected)) // transform widget must exist while composing systems are running.
        ;
    }
}